...
Kontext V2 is Ionut's primary long-term memory backend for personal/project continuity. The old `mem0` MCP name is legacy compatibility only; do not describe active Kontext calls as Mem0 in user-facing text. Legacy Mem0 is backup/rollback only unless Ionut explicitly asks to compare, diagnose parity, or restore.
...
- If Kontext is unavailable, the tool is missing, or search returns no relevant result, say that clearly before answering from current context.
...
- Claude Desktop / Claude.ai custom connectors do not run local Claude Code hooks. For those clients, rely on the custom connector plus this `CLAUDE.md` instruction policy.

## Kontext Primary MCP Protocol

Kontext V2 is the primary memory system for Claude/Codex. Legacy Mem0 remains available only as rollback/backup unless Ionut explicitly asks to compare, diagnose parity, or restore.

- Prefer the direct `kontext` MCP when available.
- Treat a configured `mem0` MCP name as a Kontext-backed compatibility alias only when the client uses that name for the primary connector, and call it Kontext in user-facing text.
...
- Do not use `_last_session.md` or old local memory files as the source of truth.
